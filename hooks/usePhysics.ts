...
export function usePhysics(canvasW: number, canvasH: number) {
  ...
  // Subscribe to accelerometer (native) or DeviceMotionEvent (web)
  useEffect(() => {
    if (Platform.OS !== 'web') {
      // Lazy-import so the module is never loaded on web (avoids missing-native-module error)
      import('expo-sensors').then(({ Accelerometer }) => {
        Accelerometer.setUpdateInterval(100);
        const sub = Accelerometer.addListener((d) => {
          accelRef.current = { x: d.x, y: -d.y };
        });
        return () => sub.remove();
      });
      ...
      const handler = (e: DeviceMotionEvent) => {
        ...
        if (a) accelRef.current = { x: (a.x ?? 0) / 9.8, y: -(a.y ?? 0) / 9.8 };
      };
