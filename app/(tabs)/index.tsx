...
import { decomposeTask, mapToMicroActions, getPersona } from '../../services/taskDecomposition';

const { width: W, height: H } = Dimensions.get('window');
const BARREL_Y = H - 80;

// Lightweight local heuristic — used for immediate bubble creation while AI runs
function parseInput(text: string): Pick<Task, 'title' | 'category' | 'importance' | 'urgency' | 'energyCost'> {
  ...
  if (/论文|作业|学习|考试|研究|读书/.test(lower))      category = 'study';
  else if (/创意|设计|写作|画|音乐|拍摄/.test(lower))   category = 'creative';
  else if (/恢复|休息|睡觉|放松|散步/.test(lower))       category = 'recovery';
  else if (/工作|会议|邮件|报告|项目|代码/.test(lower))  category = 'work';
  else if (/奖励|购物|游戏|庆祝|追剧/.test(lower))       category = 'reward';
...
export default function DashboardScreen() {
  ...
  const handleAddTask = useCallback(async () => {
    ...
    if (!text || isDecomposing) return;

    // 1. Parse locally so the bubble appears immediately
    ...
    try {
      // 2. Ask DeepSeek to produce ADHD-friendly micro-steps
      const result = await decomposeTask(text, parsed.category);
      const persona = getPersona(parsed.category);

      addTask({
        title: text,
        category: parsed.category,
        // Use AI values; clamp priority_weight (1-10) → importance (1-5)
        importance: Math.ceil(result.priority_weight / 2),
        urgency: parsed.urgency,
        energyCost: result.energy_cost,
        microActions: mapToMicroActions(result.steps),
        bartender_comment: result.bartender_comment,
      });

      showMascot(persona, result.bartender_comment);
    } catch (err) {
      // Fallback: create bubble with empty actions, show error hint
      addTask({ ...parsed, microActions: [] });
      showMascot('sloth', '网络有点慢，稍后双击气泡可以重新加载微动作。');
    } finally {
      setIsDecomposing(false);
    }
  }, [inputText, isDecomposing, activeTasks.length, addTask, showMascot]);
        ...
        >
          <BlurView intensity={40} tint="dark" style={styles.inputWrapper}>
            <TextInput
              ...
              multiline={false}
            />
            <TouchableOpacity onPress={handleAddTask} style={styles.sendBtn} disabled={isDecomposing}>
              {isDecomposing
